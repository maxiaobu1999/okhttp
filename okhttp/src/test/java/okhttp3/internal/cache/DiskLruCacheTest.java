...
public final class DiskLruCacheTest {
    public static final String TAG = "【DiskLruCacheTest】";
    // 允许在测试方法创建文件或者文件夹，并且在方法完成时删除创建好的文件或文件夹.当资源无法被删除时默认情况下不会抛出异常
    @Rule
    public final TemporaryFolder tempDir = new TemporaryFolder();
    // 每一个测试方法。它们的运行时间都不能超过1分钟，不然就会标志为失败。
    @Rule
    ...
    /** 日志文件的存放目录 */
    private File cacheDir;// /var/folders/2v/kjp9t_7x3mlftw2wmp9nb3jwbf_6s7/T/junit55524823145997623
    /** 日志文件 */
    private File journalFile;// /var/folders/2v/kjp9t_7x3mlftw2wmp9nb3jwbf_6s7/T/junit55524823145997623/journalFile
    /** 日志文件备份目录 */
    ...
    private void createNewCacheWithSize(int maxSize) throws IOException {
        ...
        synchronized (cache) {
            cache.initialize();// 初始化DiskLruCache
        }
        toClose.add(cache);
    }
    ...
    public void setUp() throws Exception {
        cacheDir = tempDir.getRoot();// 一个临时目录
//        System.out.println(TAG + "setUp():cacheDir=" + cacheDir);
    ...
    public void recoverFromInitializationFailure() throws IOException {
        ...
        // incomplete.
        // 添加一个未提交的条目。 这将在初始化时检测到，并且缓存将尝试删除该文件。 不要在此处显式关闭缓存，因此该条目将保留为不完整。
        // 获取key为k1的日志文件，的编辑器
        DiskLruCache.Editor creator = cache.edit("k1");

        assert creator != null;
        // 获取第一个dirty日志文件的输出流，写入数据
        BufferedSink sink = Okio.buffer(creator.newSink(0));
        sink.writeUtf8("Hello");// 写入字符串
        sink.close();
        ...
        DiskLruCache.Snapshot snapshot = cache.get("k1");// 获取缓存快照，这里是null的
//        System.out.println(TAG + "setUp():cacheDir=" + snapshot.key());
    ...
    private List<String> readJournalLines() throws Exception {
        ...
        System.out.println(TAG + "readJournalLines():result=" + result);
        source.close();
        return result;
    }
    ...
    /** 写入字符串 */
    private static void setString(DiskLruCache.Editor editor, int index, String value)
            throws IOException {
